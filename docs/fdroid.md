...
- Current release version: `versionName 1.0.1`, `versionCode 3`
...
Fastlane changelogs use the Android version code as filename, so this release uses `fastlane/metadata/android/en-US/changelogs/3.txt`.
...
The first intended F-Droid build is PocketDev 1.0.1 (`versionCode 3`) and is referenced by the immutable `v1.0.1` tag.

Before opening that merge request:

1. Ensure the release build succeeds from a clean checkout.
2. Confirm that `v1.0.1` points to the release commit containing versionName 1.0.1 and versionCode 3.
